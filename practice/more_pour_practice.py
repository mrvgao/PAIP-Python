...
def find_capacity(goal):
    return lambda states: goal in states
...
def more_pour_problem(capacities, goal, start=None):
    ...
    if start is None: start = tuple([0] * len(capacities))

    return shortest_path_search(start, psuccessors(capacities), is_goal=find_capacity(goal))
