...
def bridge_problem3(here):
    ...
    # your code here
    return lowest_cost_search((frozenset(here) | {'light'}, frozenset()),
                              bsuccessors2,
                              lambda x: not x[0] or x[0] == {'light'},
                              lambda a: max(a[:2]))  # <== your arguments here
...
def test():
    ...
    assert path == [
        ...
        ((2, 1, '->'), 17),
        (frozenset([]), frozenset([1, 10, 2, 5, 'light']))], path
    return 'test passes'
